...
double QueryPoint :: init_visibility(Rectangle Target)//considering no obstacle
{
...
void QueryPoint :: update_visibliliyRegion(Rectangle obstacle, Rectangle target)
{
...
{
    public:
    bool operator()(const Box& t1, const Box& t2) 
    {
...
Box HeapEntry_to_box(HeapEntry *he)
{
	Box obj;
	obj.a.x = he->x1;
	obj.b.x = he->x2;
...
float RTree::VCM_visibility(QueryPoint q,Rectangle T)
{
	Rectangle temp_rect;
...
float MindistBetweenBox(Box T, Box obj);
float RTree::VCM_visibility3D(QueryPoint3D q,Box T)
{
	Box temp_rect;
...
void RTree::MOV(QueryPoint q[], int num_of_query_points, Rectangle T,int k, QueryPoint k_answers[],int& page, int& obstacle_considered)
{
	int k_position=0;
	int k_original=k;
	if(k<=0) return;
...
bool IsObstacleOutsideQPoint(QueryPoint qp_check,Rectangle obj , Rectangle T)
{
	Rectangle qp_check_rect;
...
void RTree::Alternate_approach(QueryPoint q[], int num_of_query_points, Rectangle T,int k, QueryPoint k_answers[], int& page,int& obstacle_considered)
{
	int k_position=0;
	int k_original=k;
	if(k<=0) return;
...
//obstacle centric
float obstacleAffectsRegion_val(QueryPoint q, Rectangle obs, Rectangle target)
{
	QueryPoint temp = q;
...
void RTree::MOV_obs(QueryPoint q[], int num_of_query_points, Rectangle T,int k, QueryPoint k_answers[],int& page, int& obstacle_considered)
{
	int k_position=0;
	int k_original=k;
	if(k<=0) return;
...
float angleBetweenLine_Plane(Point3D p1,Point3D p2, Point3D p3, Point3D mid, Point3D l2)
{
	...
	Point3D n = unitNormalVector(p1,p2);
	...
	return angleBetween2Vectors(n, l2);
}
...
bool equationOfPlane(Point3D a, Point3D b, Point3D c, Point3D d, Plane &p)
{
...
Point3D normalVectorOfPlane(Plane p)
{
	Point3D n;
	n.x=p.A;
	n.y=p.B;
...
bool intersectionOfLine_Plane(Point3D a, Point3D b, Plane p, Point3D &intersectionP)
{
	Point3D n = normalVectorOfPlane(p);
	if(dotProductVal(a,n) == 0) return false; //line and plane are parallel

	float t;
	float t_d= p.D - p.A*a.x - p.B*a.y-p.C*a.z;
...
float QueryPoint3D::init_visibility(Box t)
{
	...
	Box target(Point3D(minx,maxy,minz),Point3D(maxx,miny,maxz));	// why change maxy and miny?
...
bool shadow(Point3D q, Box obstacle,  Rectangle3D T ,Rectangle3D& shadowOfObs)
{
	Point3D cornerPoints[8];
	...
	Box obs(Point3D(minx,maxy,minz),Point3D(maxx,miny,maxz));
	...
	for(int i = 0;i<8;i++)
	{
		if(visible[i] ==true)
		{
			...
			{
				if(isPointInsideRect(intersectionP,T.p1,T.p2,T.p3,T.p4))
				{
				...
				if(i==0){minx=maxx = intersectionP.x; miny=maxy =intersectionP.y; minz=maxz =intersectionP.z;}
				else
				{
...
bool isPointInsidePlanarShape(Point3D point, vector<line> shape)
{
	...
	for(int i=0;i<shape.size();i++)
	{
		...
		angle += angleBetween2Vectors(a,b);
	}
...
bool QueryPoint3D::IsInsideVisibleRegion(Box T,Box obstacle)
{
...
void QueryPoint3D::update_visibilityRegion(Box obstacle, Box target)
{
	Rectangle3D shadowOfObs;
	bool outside =false; 
	...
	for(int i=0;i<vsize;i++)
	{
		...
		if(shadow(position,obstacle,visiblePlanes[i].boundary,shadowOfObs) == false)
		{
			...
			//check if totally obstructed by shadow
			if(isPointInsidePlanarShape(visiblePlanes[i].boundary.p1,rect) == true)
			{
		...
		tempVisibleRegion.push_back(visiblePlanes[i]);
	}
	visiblePlanes = tempVisibleRegion;

}

float obstacleAffectsRegion_val3D(QueryPoint3D q,Box nd,Box T)
{
	QueryPoint3D temp = q;
...
void RTree::MOV3D(QueryPoint3D q[], int num_of_query_points, Box T,int k, QueryPoint3D k_answers[],int& page, int& obstacle_considered)
{
	int k_position=0;
	int k_original=k;
	if(k<=0) return;
	...
	int end=0;
	int i,j;
	map<Box,bool,CompareBox> obstacle_checked;
	...
	{	
		...
		{			
			...
			for (i = 0; i < rtn -> num_entries; i++)
			{
				...
				for(j=0;j<num_of_query_points;j++)
				{
					...
					Box tempQ(q[j].position,q[j].position);
		...
		{	
			...
			{	
				...
				{
					//check if obj is already been considered, if yes, don't consider it again;
					map<Box,bool,CompareBox>::iterator mapItr;
					mapItr=obstacle_checked.find(obj);
					if(mapItr!=obstacle_checked.end())
					{
						if(obstacle_checked.find(obj)->second==true)
...
bool IsObstacleOutsideQPoint3D(QueryPoint3D q, Box obstacle, Box T)
{
/*	float *point= (float*) malloc(sizeof(float)*3);
	...
	*/
	...
	float qdist = MindistBetweenBox(Box(q.position,q.position),obstacle);
	float obstacle_dist = MindistBetweenBox(T,obstacle);
	if(obstacle_dist>qdist)return true;
	else return false;
}

void RTree::Alternate_approach3D(QueryPoint3D q[], int num_of_query_points, Box T,int k, QueryPoint3D k_answers[], int& page,int& obstacle_considered)
{
	int k_position=0;
	int k_original=k;
	if(k<=0) return;
			...
			{	
				...
				else //not leaf node
				{
					...
					for(x=0; x<size1; x++)
					{
						...
						Box tempQ(qp_check.position,qp_check.position);
...
void RTree::MOV_obs3D(QueryPoint3D q[], int num_of_query_points, Box T,int k, QueryPoint3D k_answers[],int& page, int& obstacle_considered)
{
	int k_position=0;
	int k_original=k;
	if(k<=0) return;
...
float update_naiveVisibility(QueryPoint3D q,Box obj, Box T)
{
	QueryPoint3D temp = q;
...
void RTree::naiveMOV3D(QueryPoint3D q[], int num_of_query_points, Box T,int k, QueryPoint3D k_answers[],int& page, int& obstacle_considered)
{
	int k_position=0;
	int k_original=k;
	if(k<=0) return;
...
struct Vector{
	...
	bool equla( Vector &v){ return Z( x-v.x ) && Z( y-v.y ); }
};
...
struct myRectangle
{
	...
	void build()
	{
		corner[1]=Vector( corner[2].x,corner[0].y );
        corner[3]=Vector( corner[0].x,corner[2].y );
	}
};
...
struct Polygon
{
	...
	void push( Vector v ){ P.push_back(v); return; }
...
void update_g( gpc_polygon &global,myRectangle &tar )
{
	...
	for( i=0;i<global.num_contours;i++ )
	{
		...
		{
			p.P.push_back( Vector(global.contour[i].vertex[j].x , global.contour[i].vertex[j].y) );
		}
...
double point2seg( Vector &P, LineSegment &S)
{
    Vector v = S.v1 - S.v0;
    Vector w = P - S.v0;
...
void  RTree:: rtreeVcmAlgo1( char *path )
{
	...
	//tar.corner[0] = Vector( 0,0 );
	//tar.corner[2] = Vector( 4.5,.5 );
	...
	while( !Q.empty() )
	{
		...
		for(  i=0;i<rtn->num_entries;i++ )
		{
			...
			ou.corner[0]=Vector(   min(u.bounces[0],u.bounces[1]),min(u.bounces[2],u.bounces[3]) );
			ou.corner[2]=Vector(   max(u.bounces[0],u.bounces[1]),max(u.bounces[2],u.bounces[3]) );
		...
		//	pu.print();
			...
			for( j=0;j<global_gp.num_contours;j++ )
			{
				...
				{
					pv.P.push_back( Vector( global_gp.contour[j].vertex[k].x , global_gp.contour[j].vertex[k].y  ) );
				}
				for( k=0;k<pu.size();k++ )
				{
